...
DROP FUNCTION IF EXISTS link_staff_to_facility(TEXT, TEXT); -- just in case

-- Create completely NEW function name
CREATE OR REPLACE FUNCTION admin_link_staff_member(
    target_email TEXT
)
RETURNS JSONB AS $$
DECLARE
...
-- Explicit permissions on NEW function
GRANT EXECUTE ON FUNCTION admin_link_staff_member(TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION admin_link_staff_member(TEXT) TO service_role;
