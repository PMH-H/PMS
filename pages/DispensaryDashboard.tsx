...
const DispensaryDashboard: React.FC<DispensaryDashboardProps> = ({
    ...
    const [drugForm, setDrugForm] = useState<DrugForm>({ category: 'B', default_unit: 'units' });
    ...
    const processScanCode = (code: string) => {
        if (scanContext === 'POS') {
            ...
            if (drug) {
                ...
                setDrugForm(prev => ({ ...prev, barcode: code, category: 'B', default_unit: 'units' }));
    ...
    const addDrugToCart = (drug: Drug, method: EntryMethod, overrideQty?: number, overridePrice?: number) => {
        ...
        const price = overridePrice !== undefined ? overridePrice : (drug.price_estimate || (drugBatches.length > 0 ? drugBatches[0].cost_per_unit * 1.5 : 0));
    ...
    const handleAIAutoFill = async (imageSrc: string | undefined, formType: 'DRUG' | 'BATCH') => {
        ...
        if (extracted) {
            ...
            const submitDrug = async () => {
                setFormError('');
                if (!drugForm.name || !drugForm.category || !drugForm.default_unit) {
                ...
                const newDrug: Drug = {
                    ...
                    image_front: drugForm.image_front,
                    image_back: drugForm.image_back,
                    category: drugForm.category || 'B',
                    default_unit: drugForm.default_unit || 'units',
                ...
                try {
                    ...
                    if (drugForm.initialStock && drugForm.initialStock > 0) {
                        const newBatch: DrugBatch = {
                            ...
                            received_units: drugForm.initialStock,
                            current_units: drugForm.initialStock,
                    ...
                    setDrugForm({ category: 'B', default_unit: 'units' });
                } catch (error) {
            ...
            const previewReconciliation = useMemo(() => {
                ...
                Object.entries(reconcileCounts).forEach(([batchId, count]) => {
                    ...
                    if (batch && drug) {
                        const delta = qty - batch.current_units;
                        if (delta !== 0) {
                            deltas.push({
                                batch, drug,
                                system: batch.current_units,
                                physical: qty,
                                delta,
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
                                ...
                                <div className="bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col h-full mt-6 lg:mt-0">
                                    ...
                                    <div className="flex-grow overflow-y-auto p-4 space-y-3 min-h-[200px] lg:min-h-0">
                                        ...
                                        {cart.map((item, idx) => {
                                            const d = drugs.find(drug => drug.id === item.drug_id);
                                            const isEditing = editingCartItem?.index === idx;
                                            const warning = checkExpiryWarning(item.drug_id);
                                            return (
                                                        ...
                                                        <div className="flex gap-2 items-center bg-gray-50 p-2 rounded">
                                                            <div>
                                                                ...
                                                                <input
                                                                    ...
                                                                    onChange={e => setEditingCartItem(prev => ({ ...prev!, item: { ...prev!.item, units: parseInt(e.target.value) || 1 } }))}
                                                                />
                                                            </div>
                                                            <div>
                                                                ...
                                                                {drugBatches.map(b => {
                                                                    ...
                                                                    return (
                                                                        <div key={b.id} className={`text-xs px-2 py-1 rounded flex justify-between ${isExpiring ? 'bg-red-100 text-red-800' : 'bg-gray-100'}`}>
                                                                            ...
                                                                            <span className="font-bold">{b.current_units}</span>
                                                                        </div>
                                                                    );
                                                                })}
